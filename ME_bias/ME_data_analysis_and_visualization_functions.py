...
def get_test_input_policy_single_agent(n, agent, ablation=False):
    ...
    for ME in [1, 2]:
        ...
        if ME == 1:
            listener_input = np.zeros((1, n, n), dtype=np.float32)
            listener_input[0, n - 1, :] = np.ones((1, n), dtype=np.float32)
            p_missing = []
        elif ME == 2:
            listener_input1 = np.zeros((1, n, n), dtype=np.float32)
            listener_input2 = np.zeros((1, n, n), dtype=np.float32)
            listener_input1[0, n - 1, :] = np.ones((1, n), dtype=np.float32)
            listener_input2[0, 0, :] = np.ones((1, n), dtype=np.float32)
            p_missing1 = []
            p_missing2 = []
...
def plot_rewards(agent, n=3, n_epochs=20):
    ...
    # set file path
    if agent == 'L0':
        filename1 = ('data/labeling/L0/' + str(n) + '_states/' + agent + '_1missing_rewards_run')
        filename2 = ('data/labeling/L0/' + str(n) + '_states/' + agent + '_2missing_rewards_run')
    elif agent == 'L1':
        filename1 = ('data/labeling/L1/' + str(n) + '_states/' + agent + '_1' + 'missing_5.0alpha_rewards_run')
        filename2 = ('data/labeling/L1/' + str(n) + '_states/' + agent + '_2' + 'missing_5.0alpha_rewards_run')
    if agent == 'L0_S0':
...
def plot_lexica_single_agent(agent):
    ...
    for i, n in enumerate([3, 10]):
        ...
        for j, ME in enumerate([1, 2]):
            ...
            plt.xticks(range(n), [k + 1 for k in range(n)], fontsize=18)
    ...
    fig.subplots_adjust(bottom=0.0, top=1.0, left=0.05, right=1.0,
